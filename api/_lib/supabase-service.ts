...
export async function upsertSupabaseData(table: string, data: any[], onConflict: string = 'id') {
    ...
    if (error) {
        if (error.code === 'PGRST116' || error.message.includes('schema cache') || error.message.includes('not find the table')) {
            console.warn(`Supabase Table Not Found [${cleanTable}]: Skipping upsert.`);
            return null;
        }
        if (error.message.includes('cannot insert a non-DEFAULT value into column "id"')) {
            const msg = `Supabase Error: The 'id' column in table '${cleanTable}' is set to 'GENERATED ALWAYS'. ` +
                        `Please change it to 'GENERATED BY DEFAULT' in the Supabase dashboard to allow syncing IDs from Sheets. ` +
                        `Alternatively, the Full Sync tool now uses a workaround for this.`;
            console.error(msg);
            throw new Error(msg);
        }
...
export async function fetchAllSupabaseData(table: string, select: string = '*') {
    ...
    while (hasMore) {
        ...
        if (error) {
            if (error.code === 'PGRST116' || error.message.includes('schema cache') || error.message.includes('not find the table')) {
                console.warn(`Supabase Table Not Found [${table}]: Skipping fetch.`);
                hasMore = false;
                break;
            }
